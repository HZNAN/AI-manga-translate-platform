...
async def merge_bboxes(bboxes: List[Quadrilateral], width: int, height: int) -> Tuple[List[Quadrilateral], List[List[int]]]:
    """
    Merge nearby textline bounding boxes into larger regions for OCR.
    Returns merged Quadrilaterals and the index mapping from merged to original.
    """
    G = nx.Graph()
...
def _can_merge_saber(a: Quadrilateral, b: Quadrilateral,
                     font_size_ratio_tol: float = 5,
                     aspect_ratio_tol: float = 2,
                     discard_connection_gap: float = 2,
                     char_gap_tolerance: float = 1,
                     char_gap_tolerance2: float = 3) -> bool:
    """
    Saber-Translator style merge predicate.
    Much simpler than quadrilateral_can_merge_region — no axis-aligned special case.
    font_size_ratio_tol raised to 3 because Quadrilateral.font_size is the
    short-edge of the bounding box, which inflates when furigana is included.
    """
    fs_a = a.font_size
    fs_b = b.font_size
    ...
    dist = Polygon(a.pts).distance(Polygon(b.pts))
...
def _build_text_regions(bboxes, region_indices):
    """Shared logic: from region indices build sorted textlines with colors."""
    for node_set in region_indices:
        nodes = list(node_set)
        txtlns: List[Quadrilateral] = np.array(bboxes)[nodes]
...
def merge_bboxes_text_region_saber(bboxes: List[Quadrilateral], width, height):
    """Saber-Translator style merge: relaxed predicate, same split logic."""
    G = nx.Graph()
    for i, box in enumerate(bboxes):
        G.add_node(i, box=box)

    for ((u, ubox), (v, vbox)) in itertools.combinations(enumerate(bboxes), 2):
        if _can_merge_saber(ubox, vbox):
            G.add_edge(u, v)
    ...
    yield from _build_text_regions(bboxes, region_indices)


def merge_bboxes_text_region(bboxes: List[Quadrilateral], width, height):
    """Original merge using quadrilateral_can_merge_region (strict axis-aligned checks)."""
    G = nx.Graph()
    for i, box in enumerate(bboxes):
        G.add_node(i, box=box)
...
def _regions_to_textblocks(textlines, merge_gen):
    """Convert a merge generator's output to TextBlock list."""
    total_area = sum(tl.area for tl in textlines) or 1
    text_regions: List[TextBlock] = []
    for (txtlns, fg_color, bg_color) in merge_gen:
        total_logprobs = 0
        for txtln in txtlns:
...
async def dispatch(textlines: List[Quadrilateral], width: int, height: int,
                   verbose: bool = False) -> List[TextBlock]:
    return _regions_to_textblocks(
        textlines, merge_bboxes_text_region(textlines, width, height))


async def dispatch_saber(textlines: List[Quadrilateral], width: int, height: int,
                         verbose: bool = False) -> List[TextBlock]:
    """Saber-Translator style dispatch with relaxed merge predicate."""
    return _regions_to_textblocks(
        textlines, merge_bboxes_text_region_saber(textlines, width, height))
