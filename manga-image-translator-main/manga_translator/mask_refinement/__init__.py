...
def dispatch_sync(text_regions: List[TextBlock], raw_image: np.ndarray, raw_mask: np.ndarray, method: str = 'fit_text', dilation_offset: int = 0, ignore_bubble: int = 0, verbose: bool = False, kernel_size: int = 3) -> np.ndarray:
...
async def dispatch(text_regions: List[TextBlock], raw_image: np.ndarray, raw_mask: np.ndarray, method: str = 'fit_text', dilation_offset: int = 0, ignore_bubble: int = 0, verbose: bool = False, kernel_size: int = 3) -> np.ndarray:
    ...
    return await asyncio.to_thread(
        dispatch_sync, text_regions, raw_image, raw_mask,
        method, dilation_offset, ignore_bubble, verbose, kernel_size
    )
