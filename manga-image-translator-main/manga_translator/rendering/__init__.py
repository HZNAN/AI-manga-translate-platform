...
def _calc_auto_font_size(region, font_size_minimum):
    """
    Binary search for the max font size that fits translation within the bubble.
    Inspired by Saber-Translator's calculate_auto_font_size.
    """
    ...
    N = max(1, len(region.translation.strip()))
    line_height = 1.15
    low = max(font_size_minimum, 12)
    high = max(w, h)
    best = low
    ...
    for _ in range(20):
        ...
        if region.vertical:
            chars_per_col = max(1, int(h / mid))
            cols_needed = math.ceil(N / chars_per_col)
            fits = cols_needed * mid * line_height <= w
        else:
            chars_per_line = max(1, int(w / mid))
            lines_needed = math.ceil(N / chars_per_line)
            fits = lines_needed * mid * line_height <= h
    ...
    return max(best, font_size_minimum, 12)
...
def resize_regions_to_font_size(img: np.ndarray, text_regions: List['TextBlock'], font_size_fixed: int, font_size_offset: int, font_size_minimum: int):  
    ...
    for region in text_regions: 
        ...
        # Constrain base with auto-calculated max (Saber-Translator style)
        auto_font = _calc_auto_font_size(region, font_size_minimum)
        current_base_font_size = original_region_font_size
        if auto_font is not None:
            current_base_font_size = min(current_base_font_size, auto_font)

        # Apply font_size_fixed or offset on the constrained base
...
async def dispatch(
    ...
    return await asyncio.to_thread(
        dispatch_sync, img, text_regions, font_path,
        font_size_fixed, font_size_offset, font_size_minimum,
        hyphenate, render_mask, line_spacing, disable_font_border
    )
