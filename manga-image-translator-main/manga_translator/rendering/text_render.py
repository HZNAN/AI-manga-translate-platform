...
# FreeType Face 对象不是线程安全的：多个线程同时对同一 Face 调用
# set_pixel_sizes / load_char 会导致 C 级别死锁。
# 使用 threading.local 为每个线程维护独立的 Face 对象和字体缓存。
_tls = threading.local()

def _get_font_cache() -> dict:
    if not hasattr(_tls, 'font_cache'):
        _tls.font_cache = {}
    return _tls.font_cache

def _get_font_selection() -> list:
    return getattr(_tls, 'font_selection', [])

def _set_font_selection(selection: list):
    _tls.font_selection = selection

# 兼容旧代码的全局引用（仅用于主线程 / 单线程场景）
...
def set_font(font_path: str):
    ...
    faces = [get_cached_font(p) for p in selection]
    _set_font_selection(faces)
    FONT_SELECTION = faces
...
def get_char_glyph(cdpt: str, font_size: int, direction: int) -> Glyph:
    for i, face in enumerate(_get_font_selection()):
        if face.get_char_index(cdpt) == 0 and i != len(_get_font_selection()) - 1:
            continue
        if direction == 0:
...
def put_char_vertical(font_size: int, cdpt: str, pen_l: Tuple[int, int], canvas_text: np.ndarray, canvas_border: np.ndarray, border_size: int):  
    ...
    if border_size > 0:  
        ...
        stroke_radius = 64 * max(int(0.07 * font_size), 3)  # min 3px like Saber-Translator
...
def put_text_vertical(font_size: int, text: str, h: int, alignment: str, fg: Tuple[int, int, int], bg: Optional[Tuple[int, int, int]], line_spacing: int):
    ...
    if not text :
        return
    bg_size = int(max(font_size * 0.07, 3)) if bg is not None else 0
