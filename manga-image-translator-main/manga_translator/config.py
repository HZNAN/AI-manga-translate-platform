...
class DetectorConfig(BaseModel):
    ...
    """Use raw detector mask clipped to text bounding boxes for inpainting, skipping mask_refinement. Produces cleaner inpainting results with CTD detector."""
    precise_mask_dilation: int = 10
    """Dilation pixels for precise mask mode to ensure full text coverage."""

class InpainterConfig(BaseModel):
    inpainter: Inpainter = Inpainter.lama_mpe
    """Inpainting model to use"""
    inpainting_size: int = 2560
...
class OcrConfig(BaseModel):
    ...
    """Source language hint for PaddleOCR v5 language model selection (japanese, en, korean, chinese, french, etc.)"""
