...
class MangaTranslator:
    ...
    async def _translate(self, config: Config, ctx: Context) -> Context:
        ...
        if ctx.mask is None:
            ...
            precise_mask_ok = False
            if config.detector.use_precise_mask and ctx.mask_raw is not None and ctx.text_regions:
                try:
                    ctx.mask = self._build_precise_mask(config, ctx)
                    precise_mask_ok = True
                    logger.info(f'Using precise mask mode (dilation={config.detector.precise_mask_dilation})')
                except Exception as e:
                    logger.warning(f"Precise mask failed, falling back to mask_refinement: {e}")
            if not precise_mask_ok:
                try:
    ...
    def _build_precise_mask(self, config: Config, ctx: Context) -> np.ndarray:
        """
        Build a precise mask by clipping raw_mask to detected text bounding boxes,
        then optionally dilating. This skips mask_refinement entirely and preserves
        the pixel-level accuracy of the detector's raw output (similar to Saber-Translator).
        """
        img_h, img_w = ctx.img_rgb.shape[:2]
        mask_h, mask_w = ctx.mask_raw.shape[:2]

        mask_raw = ctx.mask_raw
        if mask_raw.ndim == 3:
            mask_raw = cv2.cvtColor(mask_raw, cv2.COLOR_RGB2GRAY)

        if mask_h != img_h or mask_w != img_w:
            logger.warning(f'Precise mask: resizing mask_raw from {mask_w}x{mask_h} to {img_w}x{img_h}')
            mask_raw = cv2.resize(mask_raw, (img_w, img_h), interpolation=cv2.INTER_LINEAR)

        precise = np.zeros((img_h, img_w), dtype=np.uint8)
        for region in ctx.text_regions:
            ...
            if x2 <= x1 or y2 <= y1:
                continue
            raw_region = mask_raw[y1:y2, x1:x2]
            precise[y1:y2, x1:x2] = np.maximum(precise[y1:y2, x1:x2], raw_region)

        dilation = config.detector.precise_mask_dilation
        if dilation > 0:
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (dilation * 2 + 1, dilation * 2 + 1))
            precise = cv2.dilate(precise, kernel, iterations=1)
    ...
    async def _run_mask_refinement(self, config: Config, ctx: Context):
        ...
        if mask_raw is not None:
            ...
            if mask_h != img_h or mask_w != img_w:
                logger.warning(f'Mask refinement: resizing mask_raw from {mask_w}x{mask_h} to {img_w}x{img_h}')
                mask_raw = cv2.resize(mask_raw, (img_w, img_h), interpolation=cv2.INTER_LINEAR)
        kernel = config.kernel_size if config.kernel_size else self.kernel_size
        return await dispatch_mask_refinement(ctx.text_regions, ctx.img_rgb, mask_raw, 'fit_text',
                                              config.mask_dilation_offset, config.ocr.ignore_bubble, self.verbose, kernel)
