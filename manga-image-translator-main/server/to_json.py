...
def to_translation(ctx: Context, include_inpainted: bool = False) -> TranslationResponse:
    ...
    kwargs: dict = {"translations": results}
    if debug_folder:
        kwargs["debug_folder"] = debug_folder
    if include_inpainted and inpaint is not None:
        retval, buf = cv2.imencode('.png', inpaint)
        if retval:
            kwargs["inpainted_image"] = base64.b64encode(buf).decode('utf-8')

    return TranslationResponse(**kwargs)
